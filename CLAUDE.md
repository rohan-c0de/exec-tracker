...
- **Insider CIK discovery uses loose name tokenization, with two safety nets.** `findInsiderCik` matches our display name against SEC's "Lastname Firstname Middle" form by set-comparing tokens (every token in our name must appear in SEC's). This is intentionally loose so SEC middle initials don't break matches — but it also fails on nicknames (BJ Jenkins → Brian D Jr, Jay → Jagtar, Mike → Michael, Vats → Narayanan, Richard → Ric) and can theoretically false-positive when two distinct insiders at the same issuer share tokens (e.g. two "John Smith"s). Two safety nets: (1) set `secCik` on the exec record (string, 10-digit zero-padded) to bypass discovery and pin a known-correct CIK — examples in the data are BJ Jenkins, Jay Chaudhry, Mike Rich, Richard Smith, Vats Srivatsan; (2) when more than one CIK matches, the importer returns `ambiguous: true` and refuses to write data unless an explicit `secCik` is set. If the importer ever exits with code 2 saying "ambiguous CIK match", do not bypass it — pick the right CIK from the candidate list and pin it via `secCik`. `namesMatch` has unit-test coverage at `scripts/test/names-match.test.ts` — run it after any tokenizer change to confirm the 5 nickname-fallback cases still behave as expected.
